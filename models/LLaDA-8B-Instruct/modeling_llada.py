...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if config.flash_attention:
            try:
                from flash_attn import flash_attn_func, flash_attn_varlen_func, flash_attn_with_kvcache  # type: ignore
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position, attention_mask=attention_mask, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen
            )
        else:
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position, attention_mask=attention_mask, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache = block_group(
                    x, attention_bias=attention_bias, attention_mask=attention_mask, layers_past=layers_past, use_cache=use_cache, cu_seqlens=cu_seqlens
                )
                if attn_key_values is not None:
                    assert cache is not None
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        replace_position: Optional[Cache] = None,  # This is a hack mitigation of an issue in transformers `4.39.x`
        cu_seqlens: Optional[torch.Tensor] = None,
        max_seqlen: Optional[int] = None,
