...
def union_tensor_dict(tensor_dict1, tensor_dict2):
    # 自动对齐 batch_size
    size1 = tensor_dict1.batch_size[0] if isinstance(tensor_dict1.batch_size, torch.Size) else tensor_dict1.batch_size
    size2 = tensor_dict2.batch_size[0] if isinstance(tensor_dict2.batch_size, torch.Size) else tensor_dict2.batch_size
    if size1 != size2:
        # 若一个是另一个的整数倍，则repeat_interleave
        if size1 > size2 and size1 % size2 == 0:
            repeat_factor = size1 // size2
            for k, v in tensor_dict2.items():
                if isinstance(v, torch.Tensor) and v.shape[0] == size2:
                    tensor_dict2[k] = v.repeat_interleave(repeat_factor, dim=0)
        elif size2 > size1 and size2 % size1 == 0:
            repeat_factor = size2 // size1
            for k, v in tensor_dict1.items():
                if isinstance(v, torch.Tensor) and v.shape[0] == size1:
                    tensor_dict1[k] = v.repeat_interleave(repeat_factor, dim=0)
        else:
            raise AssertionError(f"Two tensor dict must have identical batch size or be multiples. Got {size1} and {size2}")
