...
class dLLMSFTDataset(Dataset):
    """
    dLLM-friendly SFT dataset.

    - Reads one or multiple parquet files (like the original verl SFTDataset).
    - Produces items of the form:
        {
            "input_ids": torch.LongTensor(shape=(max_length,)),
            "prompt_lengths": torch.LongTensor(())  # scalar number of prompt tokens
            # if eval=True, also returns "t": float tensor
        }
    - Tokenization pads/truncates to max_length (padding="max_length", truncation=True).
    - Compatible with the provided dLLMDataCollator which expects 'input_ids' and optional 'prompt_lengths'.
    """
    ...
    def __init__(self, parquet_files: str | ListConfig, tokenizer, config, eval=False, max_samples: int = -1):
        ...
        # if eval mode, create fixed t schedule per sample (linspace 0..1)
        if self.eval:
            # Produce a tensor t per sample in [0,1]. Use float32.
            eps = 1e-3
            self.t = (1 - eps) * torch.linspace(0.0, 1.0, steps=len(self.prompts), dtype=torch.float32) + eps
