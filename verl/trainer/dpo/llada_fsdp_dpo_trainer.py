...
class DLLMRayDPOTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_data['input_ids'] = test_data['input_ids'].view(-1, test_data['input_ids'].size(-1))
            test_data['attention_mask'] = test_data['attention_mask'].view(-1, test_data['attention_mask'].size(-1))

            test_batch: DataProto = DataProto.from_single_dict(test_data)
            
            test_batch.meta_info["response_length"] = self.config.data.max_response_length
            ...
            val_loss += val_output.meta_info["metrics"]['val/loss']
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                uid_array = np.array([str(uuid.uuid4()) for _ in range(batch_dict['input_ids'].size(0))], dtype=object)
                
                batch_dict['input_ids'] = batch_dict['input_ids'].view(-1, batch_dict['input_ids'].size(-1))
                batch_dict['attention_mask'] = batch_dict['attention_mask'].view(-1, batch_dict['attention_mask'].size(-1))
   
                batch: DataProto = DataProto.from_single_dict(batch_dict)
            
                batch.non_tensor_batch["uid"] = np.repeat(uid_array, 2)
                ...
                with _timer("step", timing_raw):
                    ...
                    batch.meta_info["response_length"] = self.config.data.max_response_length
                    ...
                    with _timer("update_actor", timing_raw):
                        batch.meta_info["multi_turn"] = self.config.data.multiturn.enable
